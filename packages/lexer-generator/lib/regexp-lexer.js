...
function generateModuleFunction(lexer, opt) {
    return "function(){\n" + generateModuleCore(lexer, opt) + "\n}";
}
function generateModuleCore(lexer, opt) {
    opt = opt || {};
  ...
  // console.log('templateParm: ' + templateParm, template)
    const preSubstitution = ''
          + lexer.template.Import
          + lexer.template.Constructor
          + lexer.template.Export;
