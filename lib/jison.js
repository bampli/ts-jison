...
var Lexer      = require('ts-jison-lex');
...
generator.constructor = function Jison_Generator (grammar, opt) {
    ...
    parser.createObjectCode = readTemplate('createObjectCode');
    parser.parserType = readTemplate('parserType');
    parser.parseError = lrGeneratorMixin.parseError = readTemplate('parseError');
    parser.types = readTemplate('types');
    parser.parse = readTemplate('parse');
    parser.performActionParameters = readTemplate('performActionParameters');
    parser.performActionType = readTemplate('performActionType');

    this.processGrammar(grammar);

    if (grammar.lex) {
        this.lexer = new Lexer(grammar.lex, null, this.terminals_, {generate: true, template: opt.template});
    }
...
generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators) {
    ...
    var parameters = parser.performActionParameters;
    if (this.parseParams) parameters += ', ' + this.parseParams.join(', ');

    this.performAction = "function anonymous(" + parameters + ")" + parser.performActionType + " {\n" + actions + "\n}";
...
lrGeneratorMixin.generateModuleExpr = function generateModuleExpr () {
    ...
    out += parser.types;
    out += "\nvar parser"+module.parserType+" = "+module.moduleCode;
    out += "\n"+this.moduleInclude;
...
lrGeneratorMixin.generateModule_ = function generateModule_ () {
    ...
    return { commonCode: commonCode, moduleCode: moduleCode, parserType: parser.parserType }
};

// Generate code that represents the specified parser table
lrGeneratorMixin.generateTableCode = function (table) {
    var moduleCode = JSON.stringify(table);
    var variables = ['o=' + parser.createObjectCode];
